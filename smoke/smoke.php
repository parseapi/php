...
expectOk('sanctions', fn () => $parse->sanctions('AEROCARIBBEAN AIRLINES'), fn ($r) => ($r['sanctioned'] === true && $r['matches'][0]['list'] === 'sdn') ? null : 'expected sdn match');
expectOk('sanctions clean', fn () => $parse->sanctions('Jane Smith'), fn ($r) => ($r['sanctioned'] === false && $r['matches'] === []) ? null : 'expected no match');
